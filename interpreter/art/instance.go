...
func (d *JitDebugInfo) FindSymbol(addr uint64) (libpf.Symbol, bool) {
...
func (ai *artInstance) findJitSymbol(pc uint64, fileID host.FileID) (DexSymbol, error) {
	...
	if !exists {
		...
		return DexSymbol{FilePath: "jit", Offset: pc, Name: "JIT Symbol(Unknown)"}, fmt.Errorf("not found jit debug info for pc: %#x, fileID: %x", pc, fileID)
	}
	sym, exists := dinfo.FindSymbol(pc)
	if !exists {
		dinfo.PrintAllSymbols()
		return DexSymbol{FilePath: "jit", Offset: pc, Name: "JIT Symbol(Unknown)"}, fmt.Errorf("not found jit symbol for pc: %#x, fileID: %x", pc, fileID)
	}
	return DexSymbol{FilePath: "jit", Offset: uint64(sym.Address), Name: string(sym.Name)}, nil
}
	...
	for _, mapping := range ai.mappings {
		if uint64(dex_pc) >= mapping.Vaddr && uint64(dex_pc) < mapping.Vaddr+mapping.Length {
			...
			return DexSymbol{
				...
				Name:     "Dex Symbol(Unknown)",
			}, nil
		}
	}
	return DexSymbol{Name: "Dex Symbol(Unknown)"}, fmt.Errorf("not found dex symbol for dex_pc: %#x", dex_pc)
}
...
func (ai *artInstance) Symbolize(symbolReporter reporter.SymbolReporter,
	...
	if file == host.FileID(0) {
		...
		symbol = DexSymbol{FilePath: "Unknown JIT Info", Name: "Unknown JIT Info", Offset: pc}
		fileID = libpf.NewFileID(uint64(uintptr(unsafe.Pointer(ai))), 0xdeadbeef)
		...
		fileID = libpf.NewFileID(uint64(uintptr(unsafe.Pointer(ai))), uint64(frame.File))
	} else {
		// interpreter
	...
	frameID = libpf.NewFrameID(fileID, libpf.AddressOrLineno(symbol.Offset))
	trace.AppendFrameID(libpf.ARTFrame, frameID)
	if !symbolReporter.FrameKnown(frameID) {
		if symbol.Name == "" {
			symbol.Name = "ART Symbol(Unknown)"
		}
		log.Debugf("Add ART frame metadata, PC:%x , Symbol: %v, FrameID: %v, FileID: %v", pc, symbol.Name, frameID, fileID)
