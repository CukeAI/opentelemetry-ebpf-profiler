...
unwind_register_address(UnwindState *state, u64 cfa, u8 opcode, s32 param)
{
  ...
  case UNWIND_OPCODE_BASE_REG:
    if (opcode & UNWIND_OPCODEF_DEREF) {
      ...
      val = param >> 8;
      DEBUG_PRINT("unwind: r%d+%lu", param & 0xff, val);
      switch (param & 0xff) {
      case 28:
        addr = state->r28;
        break;
...
static ErrorCode unwind_one_frame(u64 pid, u32 frame_idx, struct UnwindState *state, bool *stop)
{
  *stop = false;
  ...
  // Try to resolve r28 for ART unwinder
  if (info->archdef3Opcode != UNWIND_OPCODE_COMMAND) {
    state->r28 = unwind_register_address(state, cfa, info->archdef3Opcode, info->archdef3Param);
    bpf_probe_read_user(&state->r28, sizeof(state->r28), (void*)(state->r28));
  }
  DEBUG_PRINT("r22: 0x%llx, r25: 0x%llx, r28: 0x%llx", (u64)state->r22, (u64)state->r25, (u64)state->r28);
